...
class EnderecoUserServiceTest {
	...
	void deletarEnderecoPorIdToken_idEnderecoExiste_NaoDeveLancarExcecao() {
		...
		Cliente cliente = clienteBuilder.id(1L).cpf("12345678994").nome("Silvana").sobrenome("Pereira da silva").dataNascimento(LocalDate.of(2000, 3, 3)).dataCriacaoConta(LocalDate.now()).email("silvana.dasilva@gmail.com").senha("{noop}123456").celular("27999833653").fotoPerfil("Qualquerfoto.jpeg").adicionarEndereco(endereco).adicionarEndereco(endereco2).getCliente();
		BDDMockito.when(usuarioAutenticadoService.getCliente()).thenReturn(cliente);
		Long idEndereco = 2L;
		BDDMockito.when(enderecoRepository.findByIdAndCliente(idEndereco, cliente)).thenReturn(Optional.of(endereco2));
	...
	void pegarEnderecosClientePorIdToken_ClienteTemEnderecosCadastrados_NaoDeveLancarExcecao() {
		...
		//ASSERT
		Assertions.assertEquals(cliente.getEnderecos().size(), listaMostrarEnderecoDTO.size());
		for (int i = 0; i < listaMostrarEnderecoDTO.size(); i++) {
		    MostrarEnderecoDTO enderecoResultado = listaMostrarEnderecoDTO.get(i);

		    Assertions.assertEquals(cliente.getEnderecos().get(i).getCep(), enderecoResultado.cep(), "O CEP do endereço deve ser igual");
		    Assertions.assertEquals(cliente.getEnderecos().get(i).getRua(), enderecoResultado.rua(), "A rua do endereço deve ser igual");
		    Assertions.assertEquals(cliente.getEnderecos().get(i).getNumero(), enderecoResultado.numero(), "O número do endereço deve ser igual");
		    Assertions.assertEquals(cliente.getEnderecos().get(i).getComplemento(), enderecoResultado.complemento(), "O complemento do endereço deve ser igual");
		    Assertions.assertEquals(cliente.getEnderecos().get(i).getBairro(), enderecoResultado.bairro(), "O bairro do endereço deve ser igual");
		    Assertions.assertEquals(cliente.getEnderecos().get(i).getCidade(), enderecoResultado.cidade(), "A cidade do endereço deve ser igual");
		    Assertions.assertEquals(cliente.getEnderecos().get(i).getEstado(), enderecoResultado.estado(), "O estado do endereço deve ser igual");
		    Assertions.assertEquals(cliente.getEnderecos().get(i).getEnderecoPadrao(), enderecoResultado.enderecoPadrao(), "A indicação de endereço padrão deve ser igual");
		}
	}
	...
	void alterarEnderecoPorIdToken() {
		...
		//ASSERT
		Assertions.assertNotEquals(cliente.getEnderecos().get(0).getCep(), mostrarEnderecoDTO.cep());
		Assertions.assertNotEquals(cliente.getEnderecos().get(0).getRua(), mostrarEnderecoDTO.rua());
		Assertions.assertNotEquals(cliente.getEnderecos().get(0).getNumero(), mostrarEnderecoDTO.numero());
		Assertions.assertNotEquals(cliente.getEnderecos().get(0).getComplemento(), mostrarEnderecoDTO.complemento());
		Assertions.assertNotEquals(cliente.getEnderecos().get(0).getBairro(), mostrarEnderecoDTO.bairro());
		Assertions.assertNotEquals(cliente.getEnderecos().get(0).getCidade(), mostrarEnderecoDTO.cidade());
		Assertions.assertEquals(cliente.getEnderecos().get(0).getEstado(), mostrarEnderecoDTO.estado());
		Assertions.assertEquals(cliente.getEnderecos().get(0).getEnderecoPadrao(), mostrarEnderecoDTO.enderecoPadrao());
	}
}
